...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            ...
            redisClient.zadd(config.coin + ':blocksFound', job.height, miner.login);
        ...
        redisClient.hincrby(config.coin + ':validShares', miner.login + '~' + miner.workerName, 1);
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (hash.toString('hex') !== resultHash) {
        ...
        redisClient.hincrby(config.coin + ':invalidShares', miner.login + '~' + miner.workerName, 1);
        return false;
    }
